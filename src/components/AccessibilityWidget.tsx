...
export default function AccessibilityWidget() {
  ...
  useEffect(() => {
    ...
    // Load theme
    const savedTheme = localStorage.getItem('saed_theme') as 'light' | 'dark' | null;
    if (savedTheme) {
      setTheme(savedTheme);
    } else {
  ...
  // Handle Light/Dark Mode DOM updates
  useEffect(() => {
    if (typeof window === 'undefined') return;
    const root = document.documentElement;
    if (theme === 'dark') {
    ...
    <div className="fixed bottom-6 right-6 z-40 font-sans select-none">
      ...
      {isOpen && (
        <div className="absolute bottom-18 right-0 w-72 rounded-3xl bg-white p-6 shadow-2xl border border-zinc-200/80 dark:bg-zinc-900 dark:border-zinc-800 animate-fade-in flex flex-col gap-5">
          ...
          {/* Option B: Light/Dark Mode (Direct & clear switcher) */}
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                ...
                className={`flex items-center justify-center gap-2 py-2.5 px-3 rounded-xl text-xs font-black transition-all border ${
                  theme === 'light'
                    ? 'bg-emerald-50 text-emerald-800 border-emerald-500 dark:bg-emerald-950/30 dark:text-emerald-400'
